...
$products = [];
$total = 0;
$query = isset($_GET['search']) ? 'search=' . urlencode($_GET['search']) : '';
$apiUrl = "http://localhost/ecommerce-goup3/api/products.php?$query";
          ...
          <div class="card h-100 shadow-sm border-0">
            ...
            <div class="position-relative">
              <a href="product.php?id=<?= $prod['id'] ?>">
                <img src="<?= $imgSrc ?>" class="card-img-top" style="height: 300px; object-fit: cover;"
                  alt="<?= htmlspecialchars($prod['name']) ?>">
              </a>
              <span
                class="badge bg-success position-absolute top-0 end-0 m-2 rounded-pill px-3 py-2 fs-6 shadow">$<?= number_format($prod['price'], 2) ?></span>
            </div>
            <div class="card-body">
              ...
              <form method="POST" action="cart_add.php">
                ...
                <div class="d-flex gap-2">
                  ...
                  <button class="btn btn-outline-primary flex-grow-1"><i class="bi bi-cart-plus me-1"></i> Add to
                    Cart</button>
                </div>
              </form>
            </div>
    ...
    <nav aria-label="Page navigation">
      <ul class="pagination justify-content-center mt-4">
        ...
        <li class="page-item <?= ($page >= ceil($total / $limit)) ? 'disabled' : '' ?>">
          <a class="page-link"
            href="?<?= isset($_GET['search']) ? 'search=' . urlencode($_GET['search']) . '&' : '' ?>page=<?= min(ceil($total / $limit), $page + 1) ?>">Next</a>
        </li>
      </ul>
    </nav>
  <?php endif; ?>
